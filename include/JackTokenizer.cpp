...
void JackTokenizer::advance(){
    ...
    if(c=='/') {
        ...
        if(symbolSetHas(c)){
            ...
            token=c;
            ifile.get(c);
            while(whiteSetHas(c)&&hasMoreTokens()) ifile.get(c);
        }else if(isdigit(c)){
            tType=INT_CONST;
            advance2();
        }else if(c=='"'){
...
void JackTokenizer::advance2(){
    while(true){
        token.push_back(c);
        ifile.get(c);
        if(whiteSetHas(c)||symbolSetHas(c)) break;
    };
    while(whiteSetHas(c)) ifile.get(c); //skip through space, \n, \t
}
...
void JackTokenizer::handelComment(){//think when sum /length /and lare next to each other
    ifile.get(c);
    if(c=='/'||c=='*'){
        if(c=='/'){
            advanceComment1();
